...
with open("hwc_table_all.csv", "r") as file:
    ...
    for row in reader:
        ...
        if name not in Habitable_List:
            Habitable_List[name] = (
                current  # Initialize a new list for planets in this solar system
            )
...
class SolarSystem:
    ...
    def __repr__(self):
        return (
            f"SolarSystem(name={self.name}, radius={self.radius}, starMass={self.starMass}, "
...
class Solar_Sys_Expo:
    def __init__(
        self, name, radius, period, home_star, star_temp_eff, dist_from_star, orb_speed
    ):
        self.name = name
        self.radius = radius
        self.period = period
    ...
    def __repr__(self):
        return (
            f"Solar_Sys_Expo(name={self.name}, radius={self.radius}, "
...
for index, row in merged_df.iterrows():
    ...
    Exo_List.append(
        Solar_Sys_Expo(pl_name, pl_rade, pl_orbper, hostname, st_teff, 0, 0)
    )
    ...
    if Item.period == 0:
        ...
        if Item.home_star not in allSolarSystems:
            ...
            if Item.star_temp_eff <= 3750:
                ...
                starMass = mass_G
            Item.dist_from_star = (
                G * starMass * (Item.period**2) / (4 * np.pi**2)
            ) ** (1 / 3)
            ...
            Item.dist_from_star = (
                G * currentStarMass * Item.period**2 / (4 * np.pi**2)
            ) ** (1 / 3)
...
solar_systems_dict = {
    key: solar_system_to_dict(value) for key, value in allSolarSystems.items()
}
habitable_worlds_dict = {
    key: habitable_exo_to_dict(value) for key, value in Habitable_List.items()
}
